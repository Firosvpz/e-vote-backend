...
export const bookingPlan = async (req, res) => {
  try {
    ...
    const newBooking = await Booking.findById(booking._id)
      .populate('userId', 'name email').populate('planId', 'planName');
    ...
    console.log('userEmail:', userEmail,' userName:', userName, 'planName:', planName, 'status:', status);
    
    await sendStatusUpdateEmail(userEmail, status, planName,userName);
    res.status(200).json({ msg: 'Plan booked successfully', plan,user,booking });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
  ...
  try {
    ...
    for (const booking of bookings) {
      ...
      if (!bookingMap[planId]) {
        bookingMap[planId] = booking.status; // only set if first time (latest)
      }
    }
